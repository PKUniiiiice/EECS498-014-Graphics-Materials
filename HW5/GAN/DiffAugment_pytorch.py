...
def rand_cutout(x, ratio=0.5):
    ...
    for b in range(B):
        mask_x_length = int(ratio * W)
        mask_y_length = int(ratio * H)

        left_up_x = np.random.randint(low=0, high=W-mask_x_length)
        left_up_y = np.random.randint(low=0, high=H-mask_y_length)

        x[b, :, left_up_y:(left_up_y + mask_y_length), left_up_x:(left_up_x+mask_x_length)] = 0
