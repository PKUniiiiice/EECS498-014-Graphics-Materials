...
def get_rays(H, W, focal, pose):
    ...
    # u is along W, i.e. from 0 to W-1
    # v is along H, i.e. from 0 to H-1
    # so first row of u,v is actually the vertical line w=0, h from 0 to H-1
    # after transpose, first row of u,v is horizontal line,  h=0
    # change u,v to camera cord
    
    cx = W / 2
    cy = H / 2

    x = torch.flatten((u - cx) / focal)
    y = torch.flatten(-(v - cy) / focal)
    z = -torch.ones(H*W)

    # [x,y,z,1]^T
    points = torch.vstack([x, y, z]).to(device, dtype=torch.float32) 
...
def positional_encoder(p, L_embed=6):
    ...
    for i in range(L_embed):
        ...
        freq = 2 ** i
        rets.append(torch.sin(freq * p))
        rets.append(torch.cos(freq * p))
...
def render(model, rays_o, rays_d, near, far, n_samples, rand=False):
    ...
    # it looks like, all ray use the same t array?
    ...
    points = rays_o.unsqueeze(-2) + (t.view(1, 1, n_samples, 1)) * rays_d.unsqueeze(-2) # dims is [# rays, # sample along each ray, 3]
    # points = ...
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    dists = torch.cat([t[1:]-t[:-1], one_e_10]).to(device)
    # exp(-sigma_i delta_i)
    # shape of sigma [# rays, # sample along each ray]
    alpha = torch.exp(-sigma * dists)
    Ti = cumprod_exclusive(alpha)
    weights = Ti*(1-alpha)

    pix_color = weights.unsqueeze(-1)*rgb
  
    # Compute the weighted sum of RGB values along each ray to get the final pixel color.

    rgb_map =  torch.sum(pix_color, dim=-2) 
