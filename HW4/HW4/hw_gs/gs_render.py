...
def build_scaling_rotation(scaling_vector, quaternion_vector):
    ...
    S.diagonal(dim1=1, dim2=2).copy_(scaling_vector)
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                # range of tile
                h_start, h_end = h, h+TILE_SIZE
                w_start, w_end = w, w+TILE_SIZE

                # check overlap
                in_mask = (g_br_x > w_start) & (g_tl_x < w_end) & (g_tl_y < h_end) & (g_br_y > h_start)
                ...
                gauss_weight = torch.exp(-0.5 * dx.unsqueeze(-2) @ sorted_inverse_conv @ dx.unsqueeze(-1)).view(dx.shape[:2])
                alpha = (gauss_weight[..., None] * sorted_opacity[None]).clip(max=0.99) # Hint: Check step 2 in the instruction pdf
                T = torch.roll(torch.cumprod(1-alpha, dim=1), shifts=1, dims=1) # Hint: Check Eq. (6) in the instruction pdf
                T[..., 0] = 1

  
                acc_alpha = (alpha * T).sum(dim=1) # Hint: Check Eq. (8) in the instruction pdf
                tile_color = (T*alpha).squeeze(-1) @ sorted_color + 1-acc_alpha # Hint: Check Eq. (5) in the instruction pdf
                tile_depth = (T*alpha).squeeze(-1) @ sorted_depths # Hint: Check Eq. (7) in the instruction pdf
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
